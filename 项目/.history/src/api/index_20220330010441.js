...
//登录接口
export const reqUserLogin = (data) => requests({ url:'/api/doLogin',data,method:'post'});

//注销接口
export const reqLogout = (data) => requests({ url:`/api/logout`,headers:data,method:'post'});
...
//获取用户信息【userId】
export const reqUserInfo1 = (userId)=>requests({url:`/api/user/${userId}`,method:'get'})
...
//获取所有角色
export const reqAllRoles = (userId,data)=>requests({url:`/api/role/role-list/${userId}`,method:"get",params:data});
...
//修改角色名
export const reqChangeRole = (roleId,roleName)=>requests({url:`/api/role/${roleId}`,method:"put",params:{roleName}});

//删除角色
export const reqDeleteRole = (userId,roleId)=>requests({url:`/api/role/${userId}/${roleId}`,method:"delete"});

//给用户赋值角色
export const reqGiveRole = (manageId,userId,roleId)=>requests({url:`/api/${manageId}/administrator/${userId}/${roleId}`,method:"post"});

//时段统计
export const reqTimeCount = (data)=>requests({url:'/api/site-order/time-statistic',method:"get",params:data});

//excel
export const reqExcel = (userId)=>requests({url:`/api/user/${userId}/excel`,method:"post",responseType:'blob'});