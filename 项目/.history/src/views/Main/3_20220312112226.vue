      ...
      <el-form-item label="预约器材" prop="equipment">
        ...
        >
          ...
          >
            ...
            <el-option v-for="item in this.allEquipment.data.list"  :key="item.id" :label="item.name" :value="`${item.id}~${item.name}`"></el-option>
          </el-select>
        </el-popover>
      </el-form-item>
...
export default {
  ...
  methods: {
    changeValue(value) {
      ...
      .then(() => {
        // this.num = "剩余数量" + this.allEquipment.data.list.currentCount;
        console.log(this.allEquipment.data.list[0].currentCount);
      })
      .catch(() => {
        this.$message.error("错误");
      });
