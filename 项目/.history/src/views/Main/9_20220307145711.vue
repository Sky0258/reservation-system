<template>
  <div>
    <button v-for="(item, index) in allSite.data.list"
          :key="item.index" @click="top(item.name.substr(4))">点击我</button>
      ...
      <el-tab-pane :label="day1" v-model="day">
        ...
        <div class="ground">
          <el-radio
            ...
            :key="item.index"
            @change="showstep(item.name.substr(4))"
            >{{ item.name }}</el-radio
          >
          <el-button
            type="primary"
...
export default {
  ...
  methods: {
    ...
    submitForm1(formName) {
      ...
      let siteName1 = "" + "羽毛球场" + (this.radioA - 1);
      let userId = this.userInfo.data.userId;
      let data = {
