<template>
  <div>
    ...
    <div
      id="line"
      style="height: 380px; width: 500px; border: 1px solid black;text-align:center"
    ></div>
    <el-pagination
      class="footer"
    ...
    allTime(){
      ...
      this.$store
        ...
        .then(() => {
          ...
          for(let index in this.timeCount.data.list){
              pieDate.push({
              name: this.timeCount.data.list[index].time.substr(0,5) + this.timeCount.data.list[index].time.substr(8,6),
              value: this.timeCount.data.list[index].count
            })
          }
          this.show(pieDate);
