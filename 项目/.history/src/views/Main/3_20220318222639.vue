      ...
      <el-form-item label="预约器材" prop="equipment">
        ...
        >
          ...
          <el-select
            ...
            @change="changeValue"
          >
            <el-option v-popover:popover v-for="item in this.allEquipment.data.list" :key="item.id" :label="item.name" :value="`${item.id}~${item.name}`"></el-option>
          </el-select>
        </el-popover>
      </el-form-item>
...
.box {
  height: 82%;
  width: 100%;
  background: url(./images/预约器材背景1.png);
