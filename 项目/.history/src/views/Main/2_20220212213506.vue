<template>
  ...
  <el-form-item label="预约器材" prop="equipment">
    <el-select v-model="sizeForm.equipment" placeholder="请选择所要预约的器材">
      <el-option label="篮球" value="shanghai"></el-option>
  ...
  <el-form-item label="器材数量" prop="count">
    <el-input-number v-model="sizeForm.count" @change="handleChange" :min="1" :max="10" label="数量" size="small"></el-input-number>
  </el-form-item>
...
export default {
    ...
    methods: {
      onSubmit() {
       ...
       resetForm(formName) {
        this.$refs[this.sizeForm].resetFields();
      },
      handleChange(value) {
        console.log(value);
      },
