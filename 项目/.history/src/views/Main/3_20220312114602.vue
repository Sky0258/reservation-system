      ...
      <el-form-item label="预约器材" prop="equipment">
        ...
        >
          ...
          <el-select
            ...
            @change="changeValue"
          >
            <el-option slot="reference" @click="visible = !visible" v-for="item in this.allEquipment.data.list" :key="item.id" :label="item.name" :value="`${item.id}~${item.name}`"></el-option>
          </el-select>
        </el-popover>
      </el-form-item>
