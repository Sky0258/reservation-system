...
const actions = {
    ...
    //给角色赋值菜单
    async giveRoleMenu({commit},{userId,roleId,menuIdList}){
        let result = await reqGiveRoleMenu(userId,roleId,menuIdList);
        console.log(result.data);
        if(result.data.code == 200){
            return "ok";
        }else{
