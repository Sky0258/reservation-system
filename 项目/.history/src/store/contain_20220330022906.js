...
const actions = {
    ...
    //删除角色
    async deleteRole({commit},{userId,roleId}){
        let result = await reqDeleteRole(userId,roleId);
        if(result.data.code == 200){
            return "ok";
        }else{
            return Promise.reject(new Error("faile"));
    ...
    //给用户赋值角色
    async giveRole({commit},{manageId,userId,roleId}){
        let result = await reqGiveRole(manageId,userId,roleId);
        if(result.data.code == 200){
            return "ok";
        }else{
            return Promise.reject(new Error("faile"));
    ...
    //时段统计
    async timeCount({commit},data){
        let result = await reqTimeCount(data);
        if(result.data.code == 200){
            commit("TIMECOUNT",result.data);
            return "ok";
        }else{
            return Promise.reject(new Error("faile"));
        }
    ...
    //导出
    async Excel({commit},userId){
        let result = await reqExcel(userId);
        console.log(result);
        if(result.status== 200){
            commit("EXCEL",result.data);
            return "ok";
        }else{
            return Promise.reject(new Error("faile"));
        }
