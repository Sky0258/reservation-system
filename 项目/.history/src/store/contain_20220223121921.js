...
import { reqAddSite,reqChangeSite,reqDeleteSite,reqAllSite,reqChangeEquipment,reqAddEquipment,reqDeleteEquipment,reqAllEquipment,reqCancelSite,reqCancelEquipment,reqEquipmentAllList,reqSiteAllList,reqSiteOrderList,reqEquipmentOrderList,reqOrderEquipment,reqCreateManager, reqDeleteManager, reqManagerInfo, reqMenu,reqUserChange,reqUserPass, reqUsers } from "../api";
...
const actions = {
    ...
    //获取所有场地
    async allSite({commit},{userId,categoryId}){
        let result = await reqAllSite(userId,categoryId);
        console.log(result.data);
        if(result.data.code == 200){
