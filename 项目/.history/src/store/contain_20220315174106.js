...
import { reqAllRoles,reqOrdered,reqOrderSite,reqUserRole,reqDeleteNotice,reqChangeNotice,reqAddNotice,reqNotice,reqAllNotice,reqAddSite,reqChangeSite,reqDeleteSite,reqAllSite,reqChangeEquipment,reqAddEquipment,reqDeleteEquipment,reqAllEquipment,reqCancelSite,reqCancelEquipment,reqEquipmentAllList,reqSiteAllList,reqSiteOrderList,reqEquipmentOrderList,reqOrderEquipment,reqCreateManager, reqDeleteManager, reqManagerInfo, reqMenu,reqUserChange,reqUserPass, reqUsers } from "../api";
...
const actions = {
    ...
    //获取所有角色
    async allRoles({commit},userId){
        let result = await reqAllRoles(userId);
        console.log(result.data);
        if(result.data.code == 200){
