...
const actions = {
    ...
    //给用户赋值角色
    async giveRole({commit},{managerId,userId,roleId}){
        let result = await reqGiveRole(managerId,userId,roleId);
        console.log(result.data);
        if(result.data.code == 200){
            return "ok";
        }else{
