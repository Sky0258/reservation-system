...
import { reqCancelEquipment,reqEquipmentAllList,reqSiteAllList,reqSiteOrderList,reqEquipmentOrderList,reqOrderEquipment,reqCreateManager, reqDeleteManager, reqManagerInfo, reqMenu,reqUserChange,reqUserPass, reqUsers } from "../api";
...
const actions = {
    ...
    //用户取消场地预约
    async cancelEquipment({commit},{userId,siteOrderId}){
        let result = await reqCancelEquipment(userId,siteOrderId);
        console.log(result.data);
        if(result.data.code == 200){
            // commit("EQUIPMENTALLLIST",result.data);
            return "ok";
